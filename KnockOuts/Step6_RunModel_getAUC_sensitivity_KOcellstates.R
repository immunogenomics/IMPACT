...
cellstates <- c("Th1","Th2","Th17","Treg")
...
for (tf in 1:numTFs){
	...
	#KnockOut Filtering
	#For each TF, remove features from the cell-states of that TF. 
	w <- which((FeatureTypes$cell_type_clean == cellstates[tf])|(FeatureTypes$cell_type_clean == paste0(cellstates[tf],"_stim")))	
	...
	write.table(Sens_AUC_collection, paste0("/tiffany/IMPACT_manuscript/KnockOuts/Sens_AUC_collection_",TFs[tf],"_KOcellstatefeatures.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
}
