...
distalparam <- c(20,100,200,400,1000,2000,20000)
numiters <- 10
TFs <- c("Tbet","Gata3","Stat3","Foxp3")
numTFs <- length(TFs) #Tbet Th1, Gata3 Th2, Stat3 Th17, Foxp3 Treg
for (tf in 1:numTFs){
Sens_collection <- matrix(0,length(distalparam),numiters)
AUC_collection <- matrix(0,length(distalparam),numiters)
for (dp in 1:length(distalparam)){
	...
	regionaldists <- c(-distalparam[dp],distalparam[dp])
	for (i in 1:2){
		for (j in 1:2){
	...
	Sens_collection[dp,iter] <- Sens_AUC[1,]
	AUC_collection[dp,iter] <- Sens_AUC[2,]
	}
}
	write.table(Sens_collection, paste0("/tiffany/IMPACT_manuscript/KnockOuts/Sens_collection_",TFs[tf],"_changeDistal.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
	write.table(AUC_collection,paste0("/tiffany/IMPACT_manuscript/KnockOuts/AUC_collection_",TFs[tf],"_changeDistal.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
}
