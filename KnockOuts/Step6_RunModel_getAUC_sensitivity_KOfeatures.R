...
FeatureTypes <- read.table("/tiffany/IMPACT_manuscript/KnockOuts/SupplementaryTable1_Features.txt",sep = "\t", header = T, stringsAsFactors = FALSE)
numiters <- 10
TFs <- c("Tbet","Gata3","Stat3","Foxp3")
kofeatures <- c("H3K4me1","H3K27ac","DNase","ATAC","Promoter","H3K4me3","H3K9ac","H3K27me3")
numTFs <- length(TFs) #Tbet Th1, Gata3 Th2, Stat3 Th17, Foxp3 Treg
for (tf in 1:numTFs){
Sens_collection <- matrix(0,length(kofeatures),numiters)
AUC_collection <- matrix(0,length(kofeatures),numiters)
	...
	hg19_sizes <- read.table("/tiffany/IMPACT_manuscript/KnockOuts/hg19.sizes", sep = "\t", header = F, stringsAsFactors = FALSE)
	...
	for (kof in 1:length(kofeatures)){
	#CV
	numiters <- 10
	...
	#KnockOut Filtering
	#Remove features of the type indicated in kofeatures 
	w <- which((FeatureTypes$mark == kofeatures[kof]))	
	#first two columns are label and conservation so shift this value by 2
	w_shift <- w + 2 #local
	numfeat <- (ncol(classifier)-2)/2
	w_distal <- w_shift + numfeat #distal, add length of total features (397) (minus label and cons)  
	classifier <- classifier[,-c(w_shift,w_distal)]
	...
	Sens_collection[kof,iter] <- Sens_AUC[1,]
        AUC_collection[kof,iter] <- Sens_AUC[2,]
	}
}
	#write.table(Sens_AUC_collection, paste0("Sens_AUC_collection_",TFs[tf],"_KOfeatures.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
	write.table(Sens_collection, paste0("/tiffany/IMPACT_manuscript/KnockOuts/Sens_collection_",TFs[tf],"_kofeatures.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
        write.table(AUC_collection,paste0("/tiffany/IMPACT_manuscript/KnockOuts/AUC_collection_",TFs[tf],"_kofeatures.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
