phastcons46way <- read.table("/tiffany/IMPACT_manuscript/Features/phastCons46wayPrimates.txt.gz", sep = "\t", header = F, stringsAsFactors = FALSE)
...
for (mastertf in 1:numTFs){
        print(mastertf)
	pos_bed <- read.table(paste0("/tiffany/IMPACT_manuscript/Training/train_test_positive_bed_",TFs[mastertf],"only_center.txt"),sep = "\t", header = F, stringsAsFactors = FALSE)
	neg_bed <- read.table(paste0("/tiffany/IMPACT_manuscript/Training/train_test_negative_bed_",TFs[mastertf],"only_center.txt"),sep = "\t", header = F, stringsAsFactors = FALSE)
...
write.table(AvgConservationScores, paste0("/tiffany/IMPACT_manuscript/Features/ExtraFeatures_update_",TFs[mastertf],"only_center.txt"), sep = "\t", quote = F, row.names = FALSE, col.names = FALSE)
}
