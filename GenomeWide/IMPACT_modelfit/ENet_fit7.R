...
#a list of TFs on which an IMPACT model will be learned
...
load("/tiffany/IMPACT_manuscript/Features/Features.Rdata")
...
for (mastertf in 1:numTFs){
	print(mastertf)
	      ...
	      ExtraFeatures <- read.table(paste0("/tiffany/IMPACT_manuscript/Features/ExtraFeatures_update_",args[mastertf],"only_center.txt"),sep = "\t", header = F, stringsAsFactors = FALSE)
	      positive_bed <- read.table(paste0("/tiffany/IMPACT_manuscript/Training/train_test_positive_bed_",args[mastertf],"only_center.txt"),sep = "\t", header = F, stringsAsFactors = FALSE)
	      negative_bed <- read.table(paste0("/tiffany/IMPACT_manuscript/Training/train_test_negative_bed_",args[mastertf],"only_center.txt"),sep = "\t", header = F, stringsAsFactors = FALSE)
	...
	hg19_sizes <- read.table("/tiffany/IMPACT_manuscript/GenomeWide/GenomeTracks/hg19.sizes", sep = "\t", header = F, stringsAsFactors = FALSE)
        ...
        for (i in 1:2){
                ...
                dummy[w,3] <- 2}
                m <- match(dummy[,1],hg19_sizes[,1]) #check if downstream is past chromosome end 
		...
		if (length(w)>0){
      	...
      	assign(paste0(args[mastertf],"_IMPACT_fit"), ENet_fit)	
}


tosave <- c()
for (w in 1:length(args)){

 tosave <- c(tosave,which(ls(1) == paste0(args[mastertf],"_IMPACT_fit")))

}

save(list = ls(1)[tosave], file = "/tiffany/IMPACT_manuscript/GenomeWide/IMPACT_modelfit/IMPACT_model_fits_7.RData", envir = .GlobalEnv)
